...
public static class AC_Utils
{
	...
	[MethodImpl( MethodImplOptions.AggressiveInlining )]
	public static float ConvertPixelsToDips( int pixels, float dpi ) => pixels * 96f / dpi;
	...
	internal unsafe static uint GetVendorIdFromDevice<T>( T pDevice ) where T : ID3D12Device {
		...
		try {
			...
			var hr = PInvoke.CreateDXGIFactory2( 0u, typeof(IDXGIFactory7).GUID, out object? factoryObj );
			if( hr.Failed )
